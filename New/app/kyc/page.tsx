...
export default function KYCPage() {
  ...
  const filteredKYC = mockKYCData.filter(
    (item) =>
  ...
  const totalOfficials = mockKYCData.length;
  const verifiedCount = mockKYCData.filter((item) => item.kycStatus === 'Verified').length;
  const pendingCount = mockKYCData.filter((item) => item.kycStatus === 'Pending').length;
  const flaggedCount = mockKYCData.filter((item) => item.kycStatus === 'Flagged').length;
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-600">Verification of public officials, tax clearances, and conflict of interest disclosures</p>
        </div>
      ...
      <div className="p-6 bg-white rounded-xl border border-slate-200 shadow-sm overflow-x-auto">
        ...
        {filteredKYC.length === 0 && (
          <div className="py-8 text-center text-slate-500">
            No official KYC records found matching your search.
          </div>
        )}
      </div>
    </div>
